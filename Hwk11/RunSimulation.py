...
def Simulate():
    ...
    # Simulation Part
    phi0 = np.pi/4
    x0 = np.array([0, 0, 0, 0, .5, phi0, 0])
    xhat0 = np.array([1, 1, 1, 1, 1, phi0 +.5, 1])
    v_d = 1
    omega_d = 0.25
    A, B, dynamics = createDynamics()
    segway = Segway(v_d, omega_d, A, B, dynamics, np.array([-1.5, -2, -2.5, -3]))
    ...
    simulation.pythonODE(segway.x_dot_w_observer, np.concatenate((x0, xhat0)))
